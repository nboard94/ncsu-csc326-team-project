...
public class ERController {
     ...
     * @PreAuthorize ( "hasRole('ROLE_ER')" ) public String viewDemographics (
     * final Model model ) { final User self = User.getByName(
     * SecurityContextHolder.getContext().getAuthentication().getName() ); final
     * PersonnelForm form = new PersonnelForm( self ); model.addAttribute(
     * "PersonnelForm", form ); LoggerUtil.log(
     * TransactionType.VIEW_DEMOGRAPHICS, self ); return "/er/editDemographics";
     * }
     */

    /**
     * Processes the Edit Demographics form for an Emergency Responder
     *
     * @param form
     ...
     * @PreAuthorize ( "hasRole('ROLE_ER')" ) public String demographicsSubmit
     * ( @Valid @ModelAttribute ( "PersonnelForm" ) final PersonnelForm form,
     * final BindingResult result, final Model model ) { Personnel p = null; try
     * { p = new Personnel( form ); p.setSelf( User.getByName(
     * SecurityContextHolder.getContext().getAuthentication().getName() ) ); }
     * catch ( final Exception e ) { e.printStackTrace( System.out );
     * result.rejectValue( "dateOfBirth", "dateOfBirth.notvalid",
     * "Expected format: MM/DD/YYYY" ); } if ( result.hasErrors() ) {
     * model.addAttribute( "PatientForm", form ); return "/er/editDemographics";
     * } else { // Delete the patient so that the cache has to refresh. final
     * Personnel oldPersonnel = Personnel.getByName( p.getSelf().getUsername()
     * ); if ( oldPersonnel != null ) { oldPersonnel.delete(); } p.save();
